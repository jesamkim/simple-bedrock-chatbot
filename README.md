...
- **웹 검색 기반 응답**: "마비노기 모바일이 무엇인가요?", "파이썬 가상환경 설정 방법" 등 최신 정보가 필요한 질문에 웹 검색 결과 활용
...
2. 사이드바의 "Model Selection"에서 모델 선택
   - **Claude 3.7 Sonnet**: 높은 정확도와 문맥 이해력

3. 작동 모드 선택
   - **기본 모드**: 일반 챗봇 기능 수행
   - **MCP 모드**: 웹 검색 및 현재 시간/날짜 정보 제공 기능 활성화
   - **Reasoning 모드**: 복잡한 문제 해결에 특화된 사고 과정 활용 (Claude 3.7 Sonnet 전용)

4. 사이드바의 "Document Upload" 섹션에서 문서 파일 업로드
...
7. 새로운 문서로 시작하려면 'New Chat' 버튼 클릭
