...
# 로컬 서버 모듈 임포트
try:
    from duckduckgo_mcp_server import DuckDuckGoServer
except ImportError as e:
    print(f"서버 모듈 로드 중 오류: {str(e)}")
    sys.exit(1)

class DuckDuckGoMCPClient:
    """DuckDuckGo 검색을 위한 MCP 클라이언트"""
    
    def __init__(self, max_results: int = 5):
        """
        DuckDuckGoMCPClient 초기화
        ...
        try:
            # 서버 클래스 인스턴스 생성
            self.server = DuckDuckGoServer(max_results=max_results)
            print("DuckDuckGo 서버 초기화 완료")
        except Exception as e:
            print(f"DuckDuckGo 서버 초기화 중 오류: {str(e)}")
            sys.exit(1)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print("사용법: python duckduckgo_mcp_client.py <검색어>")
